...
class AsyncFirestoreChatMessageHistory(BaseChatMessageHistory):
    ...
    def _doc_ref(self) -> Optional[firestore.AsyncDocumentReference]:
        if not self._db:
            return None
        return self._db.collection(self.collection_name).document(self.session_id)
    ...
    async def aadd_graph_component(self, component: Dict[str, Any]) -> None:
        if not self._db:
            return
        async with self._lock:
            try:
                ...
                items = data.get("graphComponents", [])
                items.append(
                    {
                        "type": "graphComponent",
                        "content": component,
                ...
                logger.error(f"Error persisting graph component: {e}")
    ...
    def add_message(self, message: BaseMessage) -> None:
        raise RuntimeError("Use async method: await history.aadd_message(message)")

    def add_user_message(self, message: str) -> None:
        raise RuntimeError("Use async method: await history.aadd_user_message(message)")

    def add_ai_message(self, message: str) -> None:
        raise RuntimeError("Use async method: await history.aadd_ai_message(message)")
                ...
                await self._doc_ref().set(
                    {
                        "messages": [
                            {
                                "role": MESSAGE_TYPE_MAP.get(type(m), "unknown"),
                                "content": m.content,
                                "timestamp": datetime.utcnow().isoformat(),
                            }
